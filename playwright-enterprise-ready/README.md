...
El workflow `.github/workflows/playwright-ci.yml` esta optimizado para ejecucion enterprise:

| Mejora | Estado |
|---|---|
| Actions principales en v4 | `checkout`, `setup-node`, `cache`, `upload-artifact`, `download-artifact`, `configure-pages`, `upload-pages-artifact`, `deploy-pages` |
| Cache npm | Habilitado con `actions/setup-node@v4` |
| Cache `node_modules` | Habilitado con `actions/cache@v4` |
| Cache browsers Playwright | Habilitado por navegador |
| Instalacion browsers | `npx playwright install --with-deps` |
| Permisos minimos | `contents: read` en tests; `pages: write` e `id-token: write` solo en reportes |
| Matrix strategy | Chrome y Firefox |
| Reportes aunque fallen tests | Job `report` con `if: always()` |
| Variables/secrets | `BASE_URL`, `API_URL`, `TEST_EMAIL`, `TEST_PHONE` desde secrets |

La matrix ejecuta:

```bash
npx playwright test --project=ui-chromium --grep "@smoke"
npx playwright test --project=ui-firefox --grep "@smoke"
```
